...
const Disconnect = ({account, isOpen}: {account:string, isOpen:boolean}) => {
	console.log(isOpen)
	return (
		<div className={`absolute right-14 py-1 px-16 mt-3 w-[200px] bg-main rounded-lg items-center ${isOpen ? "flex" : "hidden" } flex-col`}>
			<button className='py-1 cursor-pointer w-full flex items-center'><span className='hover:font-bold mr-1 font-semibold'>{shortner(account)}</span><i className="ri-file-copy-line"></i></button>
			<button onClick={disConnect} className='py-1 cursor-pointer w-full flex items-center'> <span className='hover:font-bold mr-1 font-semibold'>Disconnect</span><i className="ri-logout-circle-r-line"></i></button>
		</div>
	)
}
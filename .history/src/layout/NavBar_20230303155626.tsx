...
const Disconnect = ({account, isOpen}: {account:string, isOpen:boolean}) => {
	return (
		<div className={`absolute right-2 px-16 mt-2 w-[200px] bg-main rounded-lg items-center ${isOpen ? "flex" : "hidden" } flex-col`}>
			<div className='py-2'>{shortner(account)}</div>
			<div className='py-2'>Disconnect</div>
		</div>
	)
}
...
const Disconnect = ({account, isOpen}: {account:string, isOpen:boolean}) => {
	console.log(isOpen)
	return (
		<div className={`absolute right-2 py-1 px-16 mr-8 mt-2 w-[200px] bg-main rounded-lg items-center ${isOpen ? "flex" : "hidden" } flex-col`}>
			<div className='py-1 w-full'><span className='font-semibold'>{shortner(account)}</span></div>

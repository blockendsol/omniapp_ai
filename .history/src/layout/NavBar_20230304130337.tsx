...
const Disconnect = ({account, isOpen}: {account:string, isOpen:boolean}) => {
	return (
		<div className={`absolute right-14 py-1 justify-center mt-3 w-[200px] bg-main rounded-lg items-center ${isOpen ? "flex" : "hidden" } flex-col`}>
			<button className='py-1 cursor-pointer w-full w-full flex items-center'><span className='hover:font-bold mr-1 font-semibold'>{shortner(account)}</span><i className="ri-file-copy-line"></i></button>
